...
// 1) 모든 *.jar 파일에서 META-INF/jservice/javax.servlet.ServletContainerInitializer 파일이 있는지 찾는다.
// 2) javax.servletsErvletContainerInitializer 파일 안에 등록된 클래스의 인스턴스를 생성한다.
// 3) ServletContainerInitializer 구현 객체에 대해 onStartUp() 메서드를 호출한다.
//    - onStartup() 메서드를 호출할 때 파라미터 값 중에서 types 목록 값은,
//      ServletContainerInitializer 구현한 모든 클래스의 @HandleTypes에 선언된
//      해당 클래스를 상속 받거나 구현한 모든 클래스들의 목록이다.
// 4) 웹 애플리케이션을 시작한다.
