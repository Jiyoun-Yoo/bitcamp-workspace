...
public class Exam0260 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    //  터미널창에서 select * from x_board_file;로 데이터 확인 후 테스트!

    // 먼저 자식 테이블의 데이터를 지운다.
    int count = sqlSession.delete("BoardMapper.deleteBoardFile", 18);
    System.out.println(count);

    // 그런 후 부모 테이블의 데이터를 지운다.
    int count2 = sqlSession.delete("BoardMapper.deleteBoard", 18);
