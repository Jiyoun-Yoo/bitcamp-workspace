  ...
  public static void main(String[] args) throws Exception {
    ...
    try (Scanner keyScan = new Scanner(System.in);) {
      ...
      while(true) {
        ...
        if (filename.length() == 0) {
          // 그냥 엔터를 치면 null을 리턴하는게 아니라 빈 문자열을 리턴한다.
          break;
        }
        files.add(filename);
      }
    ...
    try (Connection con = DriverManager.getConnection(
      ...
      for(String filename : files) {
        ...
        fileStmt.setInt(2, /* 앞에서 입력한 게시글의 번호? */ 0);

        // 첨부파일테이블에 데이터를 입력하려면,
        // 게새글 테이블의 게시글 번호를 알아야 한다.
        // 문제는 바로 위에서 입력한 게시글의 PK가 자동 생성는 컬럼이기 때문에
        // 입력한 후 그 PK 값이 뭔지 알 수가 없다는 것이다.
        // 그래서 첨부파일 테이블에 데이터를 입력할 수 없다!

        // 해결책!
        //  - 데이터를 입력할 때 자동 생성된 PK 값을 알 수만 있다 있다면
        //    이 문제를 해결할 수 있다.
        //  - 다음 예제를 확인해보라!
