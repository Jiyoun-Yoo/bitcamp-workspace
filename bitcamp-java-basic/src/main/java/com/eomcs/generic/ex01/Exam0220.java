// 제네릭(Generic) 문법 정리 - 제네릭 파라미터 (타입을 지정하지 않은 경우)
...
public class Exam0220 {
  ...
  // 메서드에서 제네릭의 타입을 지정하지 않으면
  // 메인 메서드에 특정 타입으로 제한하는 문법이 있더라도, 그 문법은 무용지물이 된다.
  // 따라서 제네릭으로 선언된 클래스를 사용할 때는 반드시 타입 파라미터 값을 지정하라!
  //  => 제네릭 문법의 목적은 코드 안정성을 추구하는 것이다.
  //  => 원하는 타입이 아닌 다른 타입의 지정하는 오류(타입 오류)를 줄이기 위해 만든 문법이다.
  //  => 제네릭 문법의 대상은 컴파일러다.
