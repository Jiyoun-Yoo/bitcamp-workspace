// 제네릭(Generic) 문법 정리 - 제네릭 파라미터 (특정 타입을 지정한 경우)
...
public class Exam0223 {
  ...
  static void m1(ArrayList<B1> list) {
    // ArrayList는 Object 타입의 레퍼런스 배열을 만들기 때문에 모든 타입을 담을 수 있지만,
    // 컴파일 단계에에서 B1 타입인지(지정한 타입인지) 검사하고,
    // 일치하지 않으면 컴파일러는 컴파일 오류를 발생시킨다.

    // 컴파일러는 파라미터로 넘어온 ArrayList가 어떤 타입이든 간에
    // 문법적으로 B1 타입의 값만 다룬다. (B1과 B1의 자식들을 담을 수 있는 ArrayList)
