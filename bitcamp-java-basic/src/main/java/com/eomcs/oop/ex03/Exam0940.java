...
public class Exam0940 {
  ...
  static class Monitor {
    ...
    //  => 다음과 같이 파라미터의 값을 주지 않으면 파라미터 값을 받지 않는 생성자가 호출된다.
    //  예) Monitor()
    //  => 파라미터를 받지 않는 생성자(파라미터 없는 생성자)를
    //     기본 생성자(default constructor)라 부른다.
    //  => new 명령을 실행하여 인스턴스를 생성할 떄는 반드시 호출될 생성자를 지정해야 한다.
    //  예) new 클래스명();
    ...
    Monitor() {
      ...
      //  - 다만 현재의 생성자는 기본 생성자로서
      //    외부로부터 값을 받지 않기 때문에 초기화 블록과 다를 바가 없다.
    }
  ...
  public static void main(String[] args) {
    ...
    //          ...
    //        }
    //  4) 파라미터를 받지 않는 생성자(default constructor)를 호출한다.
    //    예) Monitor() {...}
    Monitor m1 = new Monitor();

    // 주의!
    //  => 다음과 같이 생성자를 지정하지 않는다면 컴파일 오류가 발생한다.
