...
public class Exam0210 {
  ...
  static int sValue;
  static void sm() {
    // 스태틱 메서드는 인스턴스 주소를 담는 this라는 내장 변수가 없다.
    // 왜? 클래스 이름을 사용하기 때문이다.
    // 설사 인스턴스를 가지고 이 메서드를 호출환다고 하더라도
    // 그 인스턴스의 주소를 저장할 this라는 변수가 없기 때문에 그 인스턴스 주소를 저장할 수 없다.
  }
  ...
  void im() {
    ...
    // 인스턴스 주소를 담은 this라는 변수가 내장되어 있다.

    // 만약 인스턴스 주소가 없이 호출하면 nullPointerException 오류가 발생한다.
    // 예)
    //    Exam0210 obj = null;
  ...
  class A {
    ...
    void m1() {
      ...
      // 구체적으로 어떤 클래스의 인스턴스 멤버를 찾을지 지정하지 않으면
      // 다음 순서로 찾는다.
      // 1) 로컬 변수를 찾는다.
      // 2) 인스턴스 변수를 찾는다.
      // 3) 스태틱 변수를 찾는다.
