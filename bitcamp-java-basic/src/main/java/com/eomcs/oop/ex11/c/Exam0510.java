// inner class : 바깥 클래스의 인스턴스를 보관할 this 변수와 생성자 I

package com.eomcs.oop.ex11.c;

class E {
  class X {
    // 인스턴스 메서드는 this라는 내장 변수에 인스턴스 주소를 보관한다.
    ...
    //  - 위와 같이 바깥 클래스의 인스턴스 주소를 저장하기 위해
    //    컴파일러가 추가한 변수를 사용하려면 다음과 같이 코드를 작성해야 한다.
    //    [문법]  바깥 클래스명.this
    //       예)  E.this

    // 인스턴스 메서드에 있는 this 변수는 인스턴스 메서드를 호출할 때 객체 주소를 저장한다.
    // 그렇다면 inner class에서 바깥 클래스의 인스턴스 주소는 언제 저장할까?
    //  - 바깥 클래스의 인스턴스를 가지고 중첩클래스의 인스턴스를 생성할 때 저장한다.
    //  - 즉 inner class의 생성자를 호출할 때 바깥 클래스의 인스턴스 주소를 파라미터로 전달한다.
    //  - 이를 위해 컴파일러는 inner class를 컴파일할 때 생성자를 변형한다.
...
public class Exam0510 {
  ...
  public static void main(String[] args) {
    ...
    E.X obj = outer.new X();
    // inner class의 인스턴스를 생성할 때 바깥 클래스의 객체 주소를 어떻게 전달할까?
