...
public class Exam0211 {
  public static void main(String[] args) {
    ...
    // Heap에 생성된 i1과 Constant Pool에 생성된 i3는 다른 인스턴스이다.

    // 그 범위 외의 수는 무조건 새 인스턴스를 생성한다.
    //  => 다루는 숫자가 너무 많기 때문에 무조건 상수 풀에 만드는 것은
    //     오히려 메모리 낭비가 심해지기 때문이다.
    //  => 상수 풀에 생성된 객체는 JVM이 종료되기 전까지 유지된다.
    //     가비지가 되지 않는다.
    //  => 그러나 heap에 생성된 객체는 주소를 잃어버리면
    //     가비지가 되기 때문에 메모리를 좀 더 효율적으로 사용할 수 없다.
