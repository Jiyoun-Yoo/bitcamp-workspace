...
public class Server0410 {
  public static void main(String[] args) throws Exception {
    ...
    System.out.print("대기열에 있는 클라이언트 중 한 개의 연결을 승인 : ");
    keyboard.nextLine(); // 엔터를 치면 승인된다.

    // 테스트1) 대기열에 클라이언트가 없을 때
    //  => accept()는 블로킹 상태에 놓인다.
    //     즉 리턴하지 않는다.

    // 큐(queue)에 대기중인 클라이언트 중 첫 번째 클라이언트 정보를 꺼내서 네트워크 연결을 승인한다.
    //  => 클라이언트가 서버에 연결을 요청하면, 서버는 대기열에 추가한다.
    //  => 서버소켓에서 연결을 승인하면 클라이언트와 통신할 수 있는 소켓을 리턴한다.
    //  => 대기열에 기다리고 있는 클라이언트가 없으면 대기열에 클라이언트가 접속할 때까지 기다린다.
    Socket socket = ss.accept();
    System.out.println("대기 중인 클라이언트 중 한 개의 클라이언트에 대해 연결 승인!");

    keyboard.nextLine(); // 엔터를 치면 연결이 해제된다.
