...
public class Exam0521 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    //  - 그래서 String 클래스는 바이트 배열의 값을 UCS2로 바꿀 때
    //    UTF-8 문자표를 사용하여 UCS2로 변환한다.
    //  - 예1)
    //      utf8.txt   =>  41 42 ea b0 80 ea b0 81
    //      UCS2       =>  0041 0042 ac00 ac01  <== 정상적으로 변환되었다.

    //  2) Windows 콘솔에서 실행 => 실패!
    //  - JVM을 실행할 때 file.encoding을 설정하지 않으면 OS의 기본 문자 집합으로 설정한다.
    //  - Windows의 기본 문자 집합은 MS949다.
    //    따라서 file.encoding 값은 MS949가 된다.
    //  - 바이트 배열은 UTF-8로 인코딩 되었는데,
    //    MS949 문자표를 사용하여 UCS2로 변환하려 하니까 잘못된 문자로 변환되는 것이다.
    //  - 해결책?
    //    JVM을 실행할 때 file.encoding 옵션에 정확하게 해당 파일의 인코딩을 설정하라.
    //    즉 utf8.txt.파일은 UTF-8로 인코딩 되었기 때문에
    //      '-Dfile.encoding=UTF-8' 옵션을 붙여서 실행해야 정상적으로 UCS2로 정상 변환된다.
