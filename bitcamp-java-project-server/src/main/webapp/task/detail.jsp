...
<c:forEach items="${project.members}" var="member">
  ...
  <c:if test="${member.state == 1 && member.no == task.owner.no}">selected</c:if>
  >${member.name}</option>
</c:forEach>
</select><br>
작업상태: 
<select name='status'>
  <c:forTokens items="준비,진행중,완료" delims="," var="stateLabel" varStatus="status">
    <option value='${status.index}' 
    ${task.status == status.index ? "selected" ""}>${statueLabel}</option>
  </c:forTokens>
</select><br>
<button>변경</button>
